...
App-specific ADIF fields preserve the internal contest occurrence, park name, and unscored flag (`APP_W4GNS_LOGGER_CONTEST_ID`, `APP_W4GNS_LOGGER_PARK_NAME`, `APP_W4GNS_LOGGER_UNSCORED`). Ten-character grids use the standard eight-character grid plus `GRIDSQUARE_EXT` / `MY_GRIDSQUARE_EXT`. All emitted text fields undergo ASCII conversion before their ADIF lengths are calculated. Untouched multiline fields are preserved during editing; ADIF remains an ASCII interchange format, not a lossless Unicode backup.
...
Every QSO can carry an IOTA (Islands on the Air) reference for the worked station, entered in the IOTA Ref field on the main QSO Entry screen (see [QSO entry](#qso-entry) above), plus a free-text Island Name on the QSO Details (`F6`) screen. Both round-trip through ADIF: `IOTA` for the reference, and the app-specific `APP_W4GNS_LOGGER_ISLAND_NAME` for the name.
