...
- Fix `dxccTable.lookup` (country/DXCC resolution) only ever splitting a portable callsign on its first `/`: a three-part call like `W4GNS/G/QRP` folded its location and modifier segments into one unsplit, usually-unresolvable string and silently fell back to the home call's own country — `W4GNS/G/QRP` resolved to the United States instead of England. Now every `/`-separated segment is a candidate, tried shortest-first after dropping pure modifiers (`P`, `M`, `QRP`, etc.), generalizing the existing two-segment "shorter side is the operating location" rule to any number of segments.
- Fix IARU HF World Championship scoring comparing the worked station's exchanged ITU zone only against a cty.dat lookup on the operator's own callsign (`stationITUZone`), ignoring what the operator actually declared as their own zone in Contest Entry's Sent exchange field — exactly what the event's own `sent_exchange_hint` documents that field as carrying. A station operating away from their callsign's cty.dat-default zone had every genuinely-same-zone QSO misclassified. Scoring now prefers the operator's own per-QSO sent-exchange text, falling back to the callsign-derived zone only when that field is blank.
- Fix POST (after-contest/backdated) mode auto-filling the POTA/IOTA Ref fields from *live* cluster spots and the live POTA activation API (both keyed off `time.Now()`), with no regard for the fact that POST logs a QSO at a typed historical Date/Time: a callsign that happens to be POTA-spotted today could silently stamp today's park reference onto a days-old contact. `autoFillPOTAReference` now no-ops entirely in POST mode.
- Fix a second QRZ response path (`qrzMapGeoMsg`, the World Map's location lookups) restoring the cached QRZ session key from any inbound response unconditionally — the QSO Entry auto-fill path (`qrzCallsignLookupMsg`) already got a request-ID-correlated version of this fix, but the map-feed path was missed, so an in-flight map lookup issued under old credentials could resurrect the old account's session after Station Setup changed credentials and cleared it. A credential-generation counter now gates the map path's session-key restoration the same way.
- Fix editing a QSO's callsign or band leaving a previously LoTW-matched confirmation (`lotw_confirmation.qso_id`, matched once at sync time and never re-evaluated) still pointing at the edited row: `backfillQSOGeographyFromConfirmations` could fill the edited (now different) contact's blank grid/state/IOTA from a confirmation that no longer matches its new identity, and award stats could keep crediting it there. Changing the call or band on edit now unlinks (not deletes) any confirmation still pointing at that row.
