...
The remaining finding — the award-eligibility-modeling medium finding (same-entity rules, award cutoffs, mode/category qualification, `credit_granted` consumption) — took the review's own "narrow the framing" correction (2026-09-10, third follow-up pass) rather than the "add missing rules" alternative; see below for why. It is not security-critical; it affects correctness/accuracy of the award panel. The `main.go` concentration low finding is tracked but explicitly needs no dedicated fix (see below).

## Open findings

### ~~Medium — Award-specific eligibility is incompletely modeled~~ (resolved via framing, not modeling)

**Resolution (2026-09-10):** `stats_panel.go` now captions the panel as rough progress tracking, not sponsor-certified totals, explicitly naming the unmodeled rule classes (same-entity, date-cutoff, mode/category, operating-area). Consuming `credit_granted` (the other option this review offered) was evaluated and rejected: LoTW leaves `CREDIT_GRANTED` blank for any operator who hasn't paid ARRL's separate DXCC membership fee, even when the underlying QSL is genuinely confirmed — gating the confirmed count on it would silently zero out a common, legitimate case instead of improving accuracy, and the per-award rule sets (cutoffs, category/mode qualification, operating-area limits) still require real modeling work this pass didn't attempt. The table below is left as-is as the definitive list of what would need to be built if a future pass pursues full modeling instead.
