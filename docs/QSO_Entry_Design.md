...
Persistent files use stable per-user locations so launching from another directory does not create a second log. Unless `CWLOGGER_DB` overrides it or an existing legacy `./w4gns.db` is found, the database lives under `$XDG_DATA_HOME/cwlogger/` (normally `~/.local/share/cwlogger/`). Configuration lives under `$XDG_CONFIG_HOME/cwlogger/` (normally `~/.config/cwlogger/`). When the application launches its own `xterm` or `gnome-terminal` window, it saves the last character-cell dimensions in the configuration directory and reapplies them at the next launch; other supported terminal emulators use their own default size because no equivalent command-line option has been confirmed. Running with `--in-current-terminal` does not create a new window.
